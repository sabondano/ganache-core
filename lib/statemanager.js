...
StateManager.prototype.queueTransaction = function(method, txParams, blockNumber, callback) {
  ...
  if (txParams.data != null && txParams.data !== "") {
    rawTx.data = utils.addHexPrefix(txParams.data);
  }

  // web3@1.0.0-beta.36 converts:
  // `undefined`, `null`, `"0x0"`, `"0x"`, `0`, `"0"` to `"0x"`
  // which we convert to the hex string: `""`
  // So we just treat "" as undefined/null. :-/
  // This bug is likely due to: https://github.com/ethereumjs/rlp/issues/28
  if (txParams.nonce != null && txParams.nonce !== "") {
