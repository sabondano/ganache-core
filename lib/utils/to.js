...
var FakeTransaction = require("ethereumjs-tx/fake");
...
module.exports = {
  ...
  txFromRaw: function(rawTx) {
    const isRaw = rawTx.hasOwnProperty("v") && rawTx.hasOwnProperty("r") && rawTx.hasOwnProperty("s");
    let Tx = (() => {
      const SuperClass = isRaw ? Transaction : FakeTransaction;
      return class TransactionWrapper extends SuperClass {
        constructor(rawTx) {
          super(rawTx);
          this._hashBuffer = super.hash(isRaw);
          this._hashHex = utils.addHexPrefix(this._hashBuffer.toString("hex"));
        }
        // hash(toBuffer = false) {
        //   if (toBuffer) {
        //     return this._hashBuffer;
        //   }
